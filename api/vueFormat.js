export function vueStats(JSON) {
  ...
  for (var EType in JSON) {
    if (
      EType != "Level Up" &&
      (EType == "Hinge" || EType == "Squat" || EType == "UB Hor Push")
    ) {
...
export function vueProgress(JSONStats) {
  ...
  output.coreExerciseTableItems.push(
    getVueStat("UB Hor Push", JSONStats["UB Hor Push"])
  );
  ...
  for (var EType in JSONStats) {
    if (
      ...
      output.secondaryExerciseTableItems.push(
        getVueStat(EType, JSONStats[EType])
      );
  }
  return output;
}
