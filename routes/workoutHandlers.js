...
async function saveWorkout(body, userInstance, vWID, submit = false) {
    ...
    // Check if alloy
      ...
      if (submit) {
        ...
        var setDescription =
          RepPerformance +
          " Reps x " +
          thisPattern.alloyweight +
          " lbs @ " +
          10 +
          " RPE (Alloy) " +
          allStats[_EType].Status.string;
  ...
  for (var EType in lastSets) {
    ...
    if (Val.Weight && Val.RPE && Val.Reps) {
      var setDescription =
        Val.Reps + " Reps x " + Val.Weight + " lbs @ " + Val.RPE + " RPE";
      ...
      if (lastSetPattern.stop) {
        ...
        if (lastSetPattern.specialStage == 0) {
          ...
          // Non-stop-RPE case
          if (
            lastSetPattern.sets <= maxStopSets + 1 &&
      ...
      //DROP SETS
        ...
        if (lastSetPattern.specialStage == 0) {
          lastSetPattern.specialStage += 1;
          lastSetPattern.dropWeight = Math.round(
            ((100 - lastSetPattern.specialValue) / 100) * Val.Weight
          );
          lastSetPattern.sets += 1;
          lastSetPattern.setList.push({
            SetNum: lastSetPattern.sets,
          ...
          if (
            lastSetPattern.sets <= maxDropSets &&
      ...
      console.log(
        "	Calculating new max with inputs: " +
          Val.Weight +
          ", " +
          Val.Reps +
          ", " +
